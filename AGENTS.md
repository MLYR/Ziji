...
4. R0/R1 在完成最小定向验证后可由 `IN_PROGRESS → DONE`；R2 使用 `IN_PROGRESS → VERIFYING → DONE`；R3 或明确要求独立审查的任务使用 `IN_PROGRESS → REVIEW → VERIFYING → DONE`。
...
同花顺公开接口是 V1 股票、ETF 盘后日线和基金历史净值的首选来源（CHG-MD-001，2026-09-05 用户决策弃用付费 Tushare）；手工产品、价格和净值是明确的降级路径。不得把盘后数据描述为“实时”。
...
- 外部邮件、对象存储、同花顺行情和汇率供应商必须通过端口/适配器隔离。
...
- R2/R3 实现任务必须关联 RTM 需求 ID 和 `T-*`；R0/R1 记录可执行的验收条件即可。
...
验证分为本地定向验证、变更感知 PR CI、批次门禁和发布验证，四者职责不得重复堆叠：

1. R0 低风险包括文档、文案、CSS/排版、非关键展示、测试代码自身和非行为配置；只执行 `git diff --check` 或贴近改动的静态检查，默认不要求 Testcontainers、E2E 或独立审查。
2. R1 普通业务改动包括普通 CRUD、查询、页面逻辑、非账务服务和 Mobile 常规状态处理；执行受影响模块的 check、编译或相关单元测试，不要求无关端完整测试。
3. R2 受契约或集成影响的改动包括 API、OpenAPI、数据库迁移、跨模块协作、导入和外部适配器；执行对应契约、集成、迁移或模块协作验证，并保留任务验收证据。
4. R3 高风险包括账务事实、金额、余额/持仓/收益、冲正、权限/认证/对象级授权、幂等、同步冲突、并发、数据删除、审计、安全以及 CI/workflow 供应链；必须执行定向深度验证，并由匹配角色独立审查。R3 还必须覆盖对应 PostgreSQL、事务、权限矩阵、幂等重放、并发、回滚或安全路径，不能用纯 Mock 测试替代真实边界验证。
5. 本地只验证受影响范围：R0 执行 diff 检查；Web/Mobile 执行对应 check 和相关测试；普通 Backend 执行编译或相关测试；API/OpenAPI 变化才执行契约与生成类型检查；migration 变化才执行 PostgreSQL Testcontainers、空库和上一版本升级验证。
6. 提交前检查暂存区（`git diff --cached --check`、`git diff --cached`）。PR CI 按变更路径执行；未受影响的 job 只能报告 `not applicable`，不得把未运行、失败、`skipped` 或其他提交的结果记为当前变更的验证证据。
7. 完整 Backend 回归、Playwright、Maestro、性能、恢复、真实供应商冒烟和深度依赖/许可证/镜像扫描，放在 B1～B4 批次门禁、发布候选、夜间回归或显式手动场景，不作为普通 PR 固定要求。
8. 任务自身验证与 PR 集成状态分离：无因果关系的其他模块失败单独登记缺陷，不阻止任务记录自身完成；实际合并限制以当前仓库保护设置和发布策略为准。
9. 扫描器无法运行、结果无法解析或确认存在密钥泄露、危险 workflow、HIGH/CRITICAL 漏洞时必须 fail closed；LOW/MODERATE finding、普通许可证信息或提示进入风险台账，不默认阻塞普通 PR；明确违反仓库许可证策略的结果仍可阻断相应安全门禁。
