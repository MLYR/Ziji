...
跨文档冲突先登记 `CHG-*` / `BUG-*` 对齐基线，不默认择一实现。`API契约.md` 与 OpenAPI 互不自动覆盖。对齐完成前按约束更强的一方执行（宁可阻塞，不可放行），并在 `BOARD.md` 登记冲突项。
模块实现前读目标目录 `AGENTS.md`（`backend` / `web` / `mobile` / `openapi` / `prototypes`）；根规则与模块规则同时生效。
`.spec-workflow/`、`.workbuddy/` 等 AI 工具工作区不是事实源：其中草稿可用作探索材料，落成基线必须晋升到 `doc/`，不得以其代替 PRD/RTM/STATUS/BOARD。

## 2. 任务登记
- 新功能、产品行为、API/OpenAPI、迁移、账务、权限、幂等、安全改动：开工前在 `BOARD.md` 登记一条叶子任务，完成后移出并同步 `STATUS.md`。汇总百分比在批次收口时重算，不必每次改动都重算。
- typo、格式、注释、lint、小测试修正、CI 文案、局部重构及当前任务附带修复：挂靠当前任务，不新建；无可挂靠的活动任务时直接实施并在提交信息中说明，不为小改动新建台账任务。
...
- 提交前必须 `git diff --cached --check` 并通读暂存 diff，确认无调试残留与无关改动。
...
- 批次收口中最后一行风险档（账务/权限/幂等/同步/安全）的改动需独立审查记录：由不携带实现上下文的新会话只依据 diff 与验证证据复核并给出书面结论；审查工具不可用时如实记为未闭合缺口，不得以实现者自查代替。

## 6. 注释
只为无法从代码理解的业务不变量、财务语义、安全边界、并发原因、兼容 workaround 写简洁中文注释。不写逐行复述；注释不得描述未实现功能或与实现不一致。

示例：好注释 `// 冲正后仍汇总原分录：余额重建包含全部 POSTED 分录（AGENTS §4.2-2）`；坏注释 `// 遍历分录列表求和`。
