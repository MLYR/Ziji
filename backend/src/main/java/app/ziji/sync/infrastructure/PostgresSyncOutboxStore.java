...
import app.ziji.sync.application.SyncOutboxEvent;
import app.ziji.sync.application.SyncOutboxStore;
import org.springframework.jdbc.core.JdbcTemplate;
import org.springframework.stereotype.Repository;
import tools.jackson.databind.ObjectMapper;

/** 使用 V014 subscription 与 consumer receipt 实现独立 claim、lease、重试和终态。 */
@Repository
public class PostgresSyncOutboxStore implements SyncOutboxStore {

	private final JdbcTemplate jdbc;
	private final ObjectMapper objectMapper;

	public PostgresSyncOutboxStore(JdbcTemplate jdbc, ObjectMapper objectMapper) {
		if (jdbc == null || objectMapper == null) {
			throw new IllegalArgumentException("同步 outbox 依赖不能为空。");
		}
		this.jdbc = jdbc;
	...
	public Optional<SyncOutboxClaim> claimNext(String consumerName, Instant now, Instant leaseUntil) {
		if (consumerName == null || now == null || leaseUntil == null) {
			throw new IllegalArgumentException("同步 outbox claim 参数不能为空。");
		}
		...
		if (updated != 1) {
			throw new IllegalStateException("同步 outbox receipt 抢占失败。");
		}
		try {
			@SuppressWarnings("unchecked")
			Map<String, Object> payload = objectMapper.readValue(candidate.payload(), Map.class);
			return Optional.of(new SyncOutboxClaim(consumerName, new SyncOutboxEvent(
	...
	public void markSucceeded(SyncOutboxClaim claim, Instant completedAt) {
		...
		if (updated != 1) {
			throw new IllegalStateException("同步 outbox receipt 完成状态写入失败。");
		}
	}
	...
	public void markRetryable(SyncOutboxClaim claim, Instant failedAt, Instant nextAttemptAt, String errorCode) {
		...
		if (updated != 1) {
			throw new IllegalStateException("同步 outbox receipt 可重试状态写入失败。");
		}
	}
	...
	public void markFinal(SyncOutboxClaim claim, Instant failedAt, String errorCode) {
		int updated = jdbc.update("""
			...
			claim.event().id(), claim.claimToken());
		if (updated != 1) {
			throw new IllegalStateException("同步 outbox receipt 最终失败状态写入失败。");
		}
	}
