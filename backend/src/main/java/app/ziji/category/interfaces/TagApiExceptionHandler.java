...
public final class TagApiExceptionHandler {
	...
	@ExceptionHandler(TagNotVisibleException.class)
	...
	ProblemDetail nameConflict(HttpServletRequest request, HttpServletResponse response) {
		return base(HttpStatus.CONFLICT, "标签名称已存在", "TAG_NAME_ALREADY_EXISTS", request, response);
	}

	@ExceptionHandler(TagVersionConflictException.class)
	ProblemDetail versionConflict(
		TagVersionConflictException exception,
		HttpServletRequest request,
		HttpServletResponse response) {
		ProblemDetail problem = base(HttpStatus.CONFLICT, "资源版本冲突", "VERSION_CONFLICT", request, response);
		problem.setProperty("versionConflict", java.util.Map.of(
	...
	@ExceptionHandler(TagPersistenceException.class)
