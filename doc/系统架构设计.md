...
V1 `applySyncOperations` 的 interface 只暴露 `TRANSACTION` 的 `CREATE`、`UPDATE`、`REVERSE` 三种语义操作。CREATE 的 `entityId` 是客户端生成且必须被最终持久化的 Transaction UUID；UPDATE/REVERSE 的 `entityId` 是既有已入账 Transaction，必须提供正整数 `baseVersion`。CREATE/UPDATE payload 只接受收入、支出、退款、转账，且 payload 不得重复携带 Transaction ID；UPDATE 的 replacement 类型由 Ledger 在运行时与原交易类型比对。一个批次按数组顺序逐项处理，单项失败不回滚其他项；每一项的账务、审计、outbox 与幂等终态仍在一个既有事务内原子提交。

后续 BE-SYNC-003 的 seam 固定在既有 `LedgerCommandApplicationService`：只深化该 module，使 CREATE 接受已校验的客户端 Transaction UUID，并沿用既有权限、版本、账务、审计和 outbox implementation。Sync module 不得新增 SQL、LedgerEntry 或平行账务写通道。
