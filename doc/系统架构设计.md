...
- 投影失败不能回滚已提交事实；后台根据 outbox 重试投影
...
修改 Transaction/LedgerEntry 或把投影结果作为账务事实。审计不等待 outbox 异步消费：它与 Ledger 事实同步
追加，保证交易、分录、审计和 outbox 在一次提交后同时可见。任一账务、审计、outbox 或幂等终态写入失败，
整个最外层事务回滚，不能留下四类事实中的任一部分。
...
共享账户变更需要按当前成员可见范围投递。成员被移除时，客户端收到资源访问撤销墓碑。

变更日志按 `recipient_user_id` 定向投递。账户业务变更为当前 ACTIVE 成员各生成一条可见记录；成员被移除时为该用户生成定向 `ACCESS_REVOKED`，该记录不依赖其仍拥有账户权限。新成员接受邀请后，通过账户初始化快照或 bootstrap 变更取得当前可见数据。
