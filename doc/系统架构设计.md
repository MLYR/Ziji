...
4. **外部源隔离。** 同花顺、邮件和未来汇率供应商通过端口/适配器接入。
...
| investment | Trade、持仓、成本、收益 | 同花顺 SDK 细节 |
| market-data | Instrument 映射、同花顺同步、价格/净值快照 | 持仓收益计算 |
...
LiquidityHold 不产生 LedgerEntry，也不改变资产总额或净资产。有效占用超过账面余额时允许原样返回负可用余额，余额端点固定返回 `liquidityStatus=NEGATIVE_AVAILABLE`；`NORMAL` 只表示可用余额非负，投影新鲜度不能混入流动性状态。其当前值和历史值都可由占用事实重建。
...
有效性按请求时点计算，未来 `effectiveAt` 只返回 `PENDING`，不提前计入不可用金额；到达 `expiresAt` 即逻辑 `EXPIRED`，即使最终化尚未运行，查询和释放/修订前置校验也必须按过期判断。`BE-ACC-008` 按本架构冻结的 PostgreSQL advisory-lock 模式实现 `LIQUIDITY_HOLD_EXPIRY_FINALIZER`，并复用 V006 已有的 `scheduled_job_runs` 记录任务运行状态；到期事实幂等物化为 `ended_at=expires_at`、`end_reason=EXPIRED` 并递增版本。它与手工释放/修订共享当前版本串行化，第一笔成功终止转换获胜，另一笔不能覆盖。
...
`GET /accounts/{accountId}/balance` 不是日终或 Dashboard 投影读取：它在一个 PostgreSQL 只读一致性快照中，以一个共同的精确 `asOf` 读取当前 ACTIVE membership、目标 `PRIMARY` 已入账 LedgerEntry 与有效 LiquidityHold。`account_balance_snapshots` 和 `account_liquidity_snapshots` 只能在已经证明与目标 instant 完全等价时使用；后者只含 business_date 和 calculated_at，不能单独证明任意 instant 的等价性。当前没有全局余额事实 sequence，因此该端点的 `asOfSequence=0` 是明确哨兵，不代表用户定向 change_log 的第 0 条记录；无法获得一致事实快照时返回 `500 INTERNAL_ERROR`，不能回退为旧缓存。
...
```text
客户端操作 ID（SyncOperation.operationId） UUID
idempotencyKey UUID
...
所有缓存、游标、pending 和 conflict 查询/写入均必须显式携带服务器确认的 `userId`；只有登录或刷新后以 Bearer 调用 `getCurrentUser` 并取得 `User.id`，Mobile 才能打开对应 SQLite scope 和发布 `AUTHENTICATED`。客户端不得解析 JWT 或以 SecureStore、演示数据、已有 SQLite 行猜测主体。`401`、`403` 或本机安全退出按不可恢复的当前会话拒绝处理：立即关闭 SQLite scope、清除内存 access token/session/userId，并按既有无效凭据/安全退出路径删除 refresh credential；删除失败只进入 `RECOVERABLE_ERROR`，不得恢复主体或 scope。刷新轮换确认到不同 `User.id` 也必须执行同一清理，且明确删除本轮已轮换且已持久化的 refresh credential；按 userId 隔离的缓存、游标、pending 和 conflict 不删除，只在没有已确认主体时不可访问。网络、`5xx` 与可恢复的安全存储错误保留可恢复状态但不得伪造主体或打开其他用户 scope。
...
5. 服务端返回 `APPLIED`、`DUPLICATE`、`CONFLICT`、`REJECTED` 或可在 5 秒后按同一幂等三元组重试的 `RETRYABLE`。
6. 客户端只在后续 `GET /sync/changes` 返回时推进持久化游标，并向用户展示冲突。

先拉后推可降低无意义冲突，但不能取代版本校验。

V1 `applySyncOperations` 的 interface 只暴露 `TRANSACTION` 的 `CREATE`、`UPDATE`、`REVERSE` 三种语义操作。CREATE 的 `entityId` 是客户端生成且必须被最终持久化的 Transaction UUID；UPDATE/REVERSE 的 `entityId` 是既有已入账 Transaction，必须提供正整数 `baseVersion`，替代版本和冲正 ID 继续由 Ledger 生成。CREATE/UPDATE payload 只接受收入、支出、退款、转账，且 payload 不得重复携带 Transaction ID、LedgerAccountId、LedgerEntry、借贷方向或系统科目 code；UPDATE 的 replacement 类型由 Ledger 在运行时与原交易类型比对。一个批次按数组顺序逐项处理，单项失败不回滚其他项；每一项的账务、审计、outbox 与幂等终态仍在一个既有事务内原子提交。

后续 BE-SYNC-003 的 seam 固定在既有 `LedgerCommandApplicationService`：只深化该 module，使其外部 interface 接收当前用户、客户端 Transaction UUID 和受控业务语义字段；Ledger 内部验证权限、分类、币种和金额，并按分类 UUID、科目性质和币种在同一事务惰性读取或确保 `INCOME_CATEGORY_<categoryUuid>` / `EXPENSE_CATEGORY_<categoryUuid>` 系统对方科目。既有 `(owner_user_id, code, currency)` 唯一约束负责并发收敛，Sync 调用方不获得内部科目。退款必须关联原支出并继承分类/对方科目/剩余额；Sync CREATE/UPDATE 的 `businessAt`、`businessDate`、`timezone` 均显式必填，以入队时确认的归属为准。B1 Sync 转账仅同币种、同金额，`fee=0` 时 `feeCategoryId=null`，`fee>0` 时必须提供费用分类；跨字段不变量由运行时逐项 `REJECTED`，不生成 FX_TRANSFER。BE-CAT-003 未闭合前，Sync payload 不上传 `tagIds`。Sync module 不得新增 SQL、LedgerEntry 或平行账务写通道。

### 7.3 变更日志

服务端维护单调递增变更序列。账务事务在同一事务写入 Transaction、LedgerEntry、audit 和 outbox；定向 `change_log` 由既有 Sync outbox consumer 在后续事务按 outbox 事实生成：
...
V015 的已发布历史内容以 epoch 初始化内置 `SYNC` 的 `TransactionPosted`、`TransactionReversed` required 订阅，
不得被修改。V017 只将仍保留该 V015 epoch 的两条内置订阅提升到 V017 的迁移提交时刻；该时刻是正常消费的含起点，
启动恢复只处理该起点及之后已到期的 receipt。V017 之前的 outbox 历史不得由启动恢复隐式消费，必须通过后续显式、
可审计的 backfill 订阅/选择事件。Sync 的 `ApplicationRunner` 仅在 Spring Boot 启动时执行一次到期 receipt 恢复；
它不创建 scheduler、轮询线程或跨消费者状态。

### 7.4 冲突

- 服务端不执行静默最后写入覆盖
- 冲突返回 `VERSION_CONFLICT` 的安全 `versionConflict` 摘要：当前版本、由版本一致派生的 ETag 和当前用户可见资源的相对 `resourceLocation`；不嵌入当前资源或可比较字段。首次摘要作为统一幂等专用安全引用保存，同 Key/Hash 重放原样返回，客户端重新 GET 后决定处理
- 财务交易冲突通过重新提交修改/冲正命令解决
- 权限撤销属于不可合并拒绝
- 同一批上传中的一笔 Transaction 和其分录是一个原子项

Mobile 冲突处置只通过 `SyncConflictResolutionPort` 进入 SQLite，不直接拼接服务器请求：

- `discardLocal(userId, operationId)` 同时表达“接受云端”和“放弃本地”。它先验证 pending/conflict 均属于该 `userId` 和同一 operationId，再在一个 SQLite transaction 删除二者；任何失败回滚且不得发起服务端写入。
- 用户在云端事实基础上编辑或作废后，`retryWithRevision(userId, oldOperationId, revisedOperation)` 必须使用 conflict 的 `currentVersion` 作为 `baseVersion`，生成新的客户端操作 ID（`SyncOperation.operationId`）、`Idempotency-Key` 和规范化 Hash；同一 SQLite transaction 插入新 pending、删除旧 pending/conflict。编辑 UPDATE 与作废 REVERSE 均不得复用旧三元组；`REJECTED` 不自动重试。
- `resourceLocation` 只能是精确的 `/api/v1/transactions/{UUID}`，拒绝绝对 URL、`//`、query、fragment、路径穿越和其他资源。Mobile 只提取 UUID 后调用类型化 `getTransaction`，不接受可执行或可跳转的任意 URL。

### 7.5 同步重试与统一幂等

同步上传复用 §6.1.1 的认证主体、作用域、请求 Hash 和生命周期；每个 `SyncOperation.idempotencyKey` 独立进入统一幂等服务，不能以批请求或普通 HTTP header 绕过。统一幂等作用域中的 OpenAPI operationId 固定为实际 `applySyncOperations`；这不同于同步载荷中的客户端操作 ID（`SyncOperation.operationId`）。request Hash 固定包含客户端操作 ID、`entityType`、`operationType`、实际 `entityId`、`baseVersion` 的显式值或 `NULL`、`payloadVersion` 和规范化 payload；`deviceId`、`createdAt` 不进入 Hash。不同用户、API 主版本或实际资源标识属于不同请求，同一用户复用同 Key 于不同资源或载荷时因 Hash 不同返回逐项 `REJECTED/IDEMPOTENCY_KEY_REUSED`。同 Key 同 Hash 仅安全重放首次结果并返回 `DUPLICATE`，携带首次安全的资源标识/版本或 Problem；`FAILED_FINAL` 重放首次安全 Problem。`IDEMPOTENCY_REQUEST_IN_PROGRESS` 和已确认无业务事实的 `FAILED_RETRYABLE/INTERNAL_ERROR` 返回逐项 `RETRYABLE`，固定 `retryAfterSeconds=5` 且不携带 versionConflict；客户端保留同一操作三元组，5 秒后串行重试并回到 PENDING，不能误标为终态 REJECTED。

上传响应不伪造 `changeSequence` 或 `serverCursor`：change log 仍由 outbox consumer 异步生成，客户端继续使用自己持久化的 cursor 拉取 `GET /sync/changes`。

## 8. 同花顺公开数据源与市场数据（CHG-MD-001）

### 8.1 适配器

```text
MarketDataSourcePort
├── fetchPrices（股票/ETF 日线、基金单位净值）
└── searchBasics（同花顺无公开搜索接口，恒返回空）

ThsMarketDataAdapter implements MarketDataSourcePort
```

核心模块只识别内部 Instrument、PriceSnapshot 和同步结果。同花顺公开端点无 key、无官方授权和 SLA：

- A 股/ETF 盘后日线：`http://d.10jqka.com.cn/v6/line/hs_{code}/01/last1800.js`（JS 变量包裹的 JSON）
- 场外公募基金历史单位净值：`http://fund.10jqka.com.cn/{code}/json/jsondwjz.json`

适配层只转换日期、收盘价和单位净值；原始响应不进入领域模型，只保留响应内容 Hash。
...
- 同花顺公开端点无凭据；服务端不保存任何行情数据源 Token、Key 或密码
- 适配器内实现进程内限流闸门和数据库每日调用配额计数（跨实例共享）
- 调用超限属于可观测业务失败，不进行无限重试
- 合规风险（无授权、无 SLA、接口可能变更）记录于 `doc/同花顺数据源合规评估.md`
...
| ADR-005 | 市场数据 | 同花顺公开接口适配器 + 手工兜底；TUSHARE 仅保留历史数据读取兼容 |
...
2. 行情数据源合规评估与风险记录已确认（同花顺公开接口，见 `doc/同花顺数据源合规评估.md`）。
