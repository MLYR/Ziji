...
V015 在数据库迁移提交时刻初始化内置 `SYNC` 的 `TransactionPosted`、`TransactionReversed` required 订阅；该时刻
是正常消费的含起点，启动恢复只处理该起点及之后已到期的 receipt。V015 之前的 outbox 历史不得由启动恢复隐式消费，
必须通过后续显式、可审计的 backfill 订阅/选择事件。Sync 的 `ApplicationRunner` 仅在 Spring Boot 启动时执行一次到期
receipt 恢复；它不创建 scheduler、轮询线程或跨消费者状态。
